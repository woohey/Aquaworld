...
export function TuningDrawer({
  ...
  return (
    <aside className="tuning-drawer" role="dialog" aria-label="调境" aria-modal="false">
      <div className="tuning-drawer__heading">
        <div>
          <p>Ecology Tuning</p>
          <h2>调境</h2>
        </div>
        <button type="button" className="tuning-drawer__close" onClick={onClose} aria-label="关闭调境">
          ×
        </button>
      </div>
      ...
      <label>
        搜索鱼种
        <input value={searchQuery} onChange={(event) => onSearchChange(event.target.value)} placeholder="中文名 / common name" />
      </label>
      ...
      <div className="range-filter">
        ...
        <button type="button" onClick={() => onFiltersChange({ ...filters, temperature: null })}>
          全部水温
        </button>
      </div>
      ...
      <div className="range-filter">
        ...
        <button type="button" onClick={() => onFiltersChange({ ...filters, ph: null })}>
          全部 pH
        </button>
      </div>
      ...
      <div className="range-filter">
        ...
        <button type="button" onClick={() => onFiltersChange({ ...filters, maxAdultSizeCm: null })}>
          全部体长
        </button>
      </div>
